...
class Adventure : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val arrayItems:Array<Item?> = listItems.toTypedArray()
...
class ViewPagerAdapter internal constructor(fm: FragmentManager) : FragmentPagerAdapter(fm){
    ...
    override fun getItem(position: Int): Fragment? {
        return when(position) {
            0 -> FragmentAdventureFirst()
            1 -> FragmentAdventureSecond()
            2 -> FragmentAdventureThird()
            3 -> FragmentAdventureFourth()
            4 -> FragmentAdventureFifth()
            5 -> FragmentAdventureSixth()
            else -> null
        }
    }
