...
class FragmentSideQuestsAdventure : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        view.buttonRefresh.setOnClickListener {
            for (i in 0 until sideQuestsAdventure.size) {
                sideQuestsAdventure[i] = generateSideQuest()
            }
            (view.listViewSideQuestsAdventure.adapter as SideQuests).notifyDataSetChanged()
        }

        view.buttonAdd.setOnClickListener {
            sideQuestsAdventure.add(generateSideQuest())
            (view.listViewSideQuestsAdventure.adapter as SideQuests).notifyDataSetChanged()
        }
        return view
    }
...
private class SideQuests(private var sideQuestsAdventure: MutableList<Quest>) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, viewGroup: ViewGroup?): View {
        ...
        if (convertView == null) {
            ...
            val viewHolder = ViewHolder(rowMain.textViewDescription, rowMain.textViewName, rowMain.textViewDifficulty, rowMain.textViewExperience, rowMain.textViewMoney, rowMain.imageViewPicture)
    ...
    private class ViewHolder(val textViewDescription:TextView, val textViewName:TextView, val textViewDifficulty:TextView, val textViewExperience:TextView, val textViewMoney:TextView, val imageViewPicture:ImageView)
}