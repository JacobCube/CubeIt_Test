...
val charClasses:Array<CharClass> = arrayOf(
        CharClass(ID = 0, dmgRatio = 10.0, armorRatio = 1.0, blockRatio = 0.0, hpRatio = 175.0, lifeSteal = false, staminaRatio = 100.0),       //for counting stats
        CharClass(ID = 1, dmgRatio = 0.75, armorRatio = 1.0, blockRatio = 20.0, hpRatio = 1.42, lifeSteal = true, staminaRatio = 2.0),          //vampire
        CharClass(ID = 2, dmgRatio = 1.5, armorRatio = 0.6, blockRatio = 15.0, hpRatio = 1.14, lifeSteal = false, staminaRatio = 1.0),          //dwarf
        CharClass(ID = 3, dmgRatio = 1.0, armorRatio = 1.0, blockRatio = 20.0, hpRatio = 1.71, lifeSteal = false, staminaRatio = 1.2),          //archer
        CharClass(ID = 4, dmgRatio = 1.25, armorRatio = 0.9, blockRatio = 10.0, hpRatio = 1.71, lifeSteal = false, staminaRatio = 1.0),         //wizard
        CharClass(ID = 5, dmgRatio = 1.25, armorRatio = 1.0, blockRatio = 5.0, hpRatio = 1.14, lifeSteal = false, staminaRatio = 1.5),          //sniper
        CharClass(ID = 6, dmgRatio = 1.0, armorRatio = 0.95, blockRatio = 45.0, hpRatio = 1.57, lifeSteal = false, staminaRatio = 1.5),         //mermaid
        CharClass(ID = 7, dmgRatio = 0.75, armorRatio = 0.9, blockRatio = 25.0, hpRatio = 1.0, lifeSteal = false, staminaRatio = 1.5),          //elf
        CharClass(ID = 8, dmgRatio = 1.25, armorRatio = 0.8, blockRatio = 0.0, hpRatio = 2.85, lifeSteal = false, staminaRatio = 0.6))          //warrior
...
data class Player(
    ...
    fun syncStats():String{
        ...
        this.health = (health * charClasses[this.charClass].hpRatio).toInt().toDouble()
        this.armor = (armor * charClasses[this.charClass].armorRatio).toInt()
        this.block = (block * charClasses[this.charClass].blockRatio).toInt().toDouble()
        this.power = (power * charClasses[this.charClass].dmgRatio).toInt()
        this.energy = (energy * charClasses[this.charClass].staminaRatio).toInt()
...
class LoadSpell(
    ...
    fun toSpell():Spell{
        return Spell(ID = this.ID, drawable = drawableStorage[this.drawable]!!,name =  this.name,energy =  this.energy,power =  this.power,fire =  this.fire,dmgOverTime =  this.dmgOverTime,level =  this.level,description =  this.description)
    }
}
...
class Spell(
    ...
    fun toLoadSpell():LoadSpell{
        return LoadSpell(ID = this.ID,drawable =  getKey(drawableStorage,this.drawable)!!,name =  this.name,energy =  this.energy, power =  this.power, fire =  this.fire, dmgOverTime =  this.dmgOverTime, level = this.level, description = this.description)
    }

    fun getStats():String{
        var text = "\n${this.name}\nLevel: ${this.level}\n ${this.description}\nStamina: ${this.energy}\nPower: ${this.power}"
        if(this.fire!=0)text+="\nFire: ${this.fire}"
        if(this.dmgOverTime!=0)text+="\nDamage over time: ${this.dmgOverTime}"
        return text
    }
}
...
data class CharClass(
        val ID:Int = 1,
        val dmgRatio:Double = 1.0,
        val hpRatio:Double = 1.0,
        val staminaRatio:Double = 1.0,
        val blockRatio:Double = 0.0,
        val armorRatio:Double = 0.0,
...
data class LoadItem(
    ...
    fun toItem():Item{
        return Item(inName = this.name, inType = this.type, inDrawable = drawableStorage[this.drawableID]!!, inQuality = this.quality, inCharClass = this.charClass, inDescription = this.description, inGrade = this.grade, inPower = this.grade,
                inArmor = this.armor, inBlock = this.block, inDmgOverTime = this.dmgOverTime, inLifesteal = this.lifeSteal, inHealth = this.health, inEnergy = this.energy, inAdventureSpeed = this.adventureSpeed, inInventorySlots = this.inventorySlots, inSlot = this.slot, inPrice = this.price)
    }
}
...
open class Item(
    ...
    fun toLoadItem():LoadItem{
        return LoadItem(name = this.name, type = this.type, drawableID = getKey(drawableStorage, this.drawable)!!, quality = this.quality, charClass = this.charClass, description = this.description, grade = this.grade, power = this.grade,
    ...
    fun getStats():String{
        ...
        if(this.dmgOverTime!=0) textView+="\nPoison: ${this.dmgOverTime}"
        if(this.lifeSteal!=0) textView+="\nBleed: ${this.lifeSteal}"
        if(this.health!=0) textView+="\nHealth: ${this.health}"
        if(this.energy!=0) textView+="\nEnergy: ${this.energy}"
        if(this.adventureSpeed!=0) textView+="\nAdventure speed: ${this.adventureSpeed}"
        if(this.inventorySlots!=0) textView+="\nInventory slots: ${this.inventorySlots}"
        return textView
    }
    fun toWearable(): Wearable {
        return Wearable(name = this.name, type = this.type, drawable = this.drawable, levelRq = this.levelRq, quality = this.quality, charClass = this.charClass, description = this.description, grade = this.grade, power = this.grade,
                armor = this.armor, block = this.block, dmgOverTime = this.dmgOverTime, lifeSteal = this.lifeSteal, health = this.health, energy = this.energy, adventureSpeed = this.adventureSpeed, inventorySlots = this.inventorySlots, slot = this.slot, price = this.price)
    }
    fun toRune(): Runes{
        return Runes(name = this.name, type = this.type, drawable = this.drawable, levelRq = this.levelRq, quality = this.quality, charClass = this.charClass, description = this.description, grade = this.grade, power = this.grade,
                armor = this.armor, block = this.block, dmgOverTime = this.dmgOverTime, lifeSteal = this.lifeSteal, health = this.health, energy = this.energy, adventureSpeed = this.adventureSpeed, inventorySlots = this.inventorySlots, slot = this.slot, price = this.price)
    }
    fun toWeapon(): Weapon{
        return Weapon(name = this.name, type = this.type, drawable = this.drawable, levelRq = this.levelRq, quality = this.quality, charClass = this.charClass, description = this.description, grade = this.grade, power = this.grade,
                armor = this.armor, block = this.block, dmgOverTime = this.dmgOverTime, lifeSteal = this.lifeSteal, health = this.health, energy = this.energy, adventureSpeed = this.adventureSpeed, inventorySlots = this.inventorySlots, slot = this.slot, price = this.price)
    }
    /*open var name:String = inName
    open var type = inType
...
fun generateItem(player:Player, inQuality: Int? = null):Item?{
    ...
    val itemTemp:Item? = when(itemReturned){
        is Weapon->Weapon(name = itemReturned.name, type = itemReturned.type, charClass = itemReturned.charClass, description = itemReturned.description, levelRq = itemReturned.levelRq, drawable = itemReturned.drawable, slot = itemReturned.slot)
        is Wearable->Wearable(name = itemReturned.name, type = itemReturned.type, charClass = itemReturned.charClass, description = itemReturned.description, levelRq = itemReturned.levelRq, drawable = itemReturned.drawable, slot = itemReturned.slot)
        is Runes->Runes(name = itemReturned.name, type = itemReturned.type, charClass = itemReturned.charClass, description = itemReturned.description, levelRq = itemReturned.levelRq, drawable = itemReturned.drawable, slot = itemReturned.slot)
        else -> Item(inName = itemReturned!!.name, inType = itemReturned.type, inCharClass = itemReturned.charClass, inDescription = itemReturned.description, inLevelRq = itemReturned.levelRq, inDrawable = itemReturned.drawable, inSlot = itemReturned.slot)
    }
...
):Item(name, type, drawable, levelRq, quality, charClass, description, grade, power, armor, block, dmgOverTime, lifeSteal, health, energy, adventureSpeed, inventorySlots, slot, price)
...
val itemsClass0:Array<Item?> = arrayOf(
        Runes(name = "Backpack", type = "Runes", drawable =  R.drawable.backpack, levelRq =  1, quality =  0, charClass =  0, description =  "Why is all your stuff so heavy?!", slot = 10)
        ,Runes(name = "Zipper", type = "Runes", drawable =  R.drawable.zipper, levelRq =  1, quality =  0, charClass =  0, description =  "Helps you take enemy's loot faster", slot = 11)
        ,Wearable(name = "Universal item 1", type =  "Wearable", drawable = R.drawable.universalitem1, levelRq =  1, quality =  0, charClass = 0, description =  "For everyone", slot = 2)
        ,Wearable(name ="Universal item 2", type =  "Wearable", drawable =  R.drawable.universalitem2, levelRq =  1, quality = 0, charClass =  0, description =  "Not for everyone", slot =  3)
)
val itemsClass1:Array<Item?> = arrayOf(
        Weapon(name = "Sword", type = "Weapon", drawable =  R.drawable.basicattack, levelRq = 1, quality = 0,charClass = 1, description = "The most sold weapon on black market", slot = 0)
        ,Weapon(name = "Shield", type = "Weapon", drawable = R.drawable.shield, levelRq = 1, quality = 0,charClass = 1, description = "Blocks 80% of next enemy attack\nYou can't use it as a boat anymore after all this", slot = 1)
        ,Wearable(name = "Belt", type ="Wearable", drawable = R.drawable.belt, levelRq = 1, quality = 0,charClass = 1, description = "I can't breath", slot = 4)
        ,Wearable(name = "Overall", type ="Wearable", drawable = R.drawable.overall, levelRq = 1, quality = 0,charClass = 1, description = "What is that smell?", slot = 5)
        ,Wearable(name = "Boots", type ="Wearable", drawable = R.drawable.boots, levelRq = 1, quality = 0,charClass = 1, description = "Can't carry it anymore", slot = 6)
        ,Wearable(name = "Trousers", type ="Wearable", drawable = R.drawable.trousers, levelRq = 1, quality = 0,charClass = 1, description = "Tight not high", slot = 7)
        ,Wearable(name = "Chestplate", type ="Wearable", drawable = R.drawable.chestplate, levelRq = 1, quality = 0,charClass = 1, description = "Chestplate protects!", slot = 8)
        ,Wearable(name = "Helmet", type ="Wearable", drawable = R.drawable.helmet, levelRq = 1, quality = 0,charClass = 1, description = "This doesn't make you any more clever", slot = 9)
)
val itemsClass2:Array<Item?> = arrayOf(
        Weapon(name = "Sword", type = "Weapon", drawable =  R.drawable.basicattack, levelRq = 1, quality = 0,charClass = 2, description = "The most sold weapon on black market", slot = 0)
        ,Weapon(name = "Shield", type = "Weapon", drawable = R.drawable.shield, levelRq = 1, quality = 0,charClass = 2, description = "Blocks 80% of next enemy attack\nYou can't use it as a boat anymore after all this", slot = 1)
        ,Wearable(name = "Belt", type ="Wearable", drawable = R.drawable.belt, levelRq = 1, quality = 0,charClass = 2, description = "I can't breath", slot = 4)
        ,Wearable(name = "Overall", type ="Wearable", drawable = R.drawable.overall, levelRq = 1, quality = 0,charClass = 2, description = "What is that smell?", slot = 5)
        ,Wearable(name = "Boots", type ="Wearable", drawable = R.drawable.boots, levelRq = 1, quality = 0,charClass = 2, description = "Can't carry it anymore", slot = 6)
        ,Wearable(name = "Trousers", type ="Wearable", drawable = R.drawable.trousers, levelRq = 1, quality = 0,charClass = 2, description = "Tight not high", slot = 7)
        ,Wearable(name = "Chestplate", type ="Wearable", drawable = R.drawable.chestplate, levelRq = 1, quality = 0,charClass = 2, description = "Chestplate protects!", slot = 8)
        ,Wearable(name = "Helmet", type ="Wearable", drawable = R.drawable.helmet, levelRq = 1, quality = 0,charClass = 2, description = "This doesn't make you any more clever", slot = 9)
)

val itemsClass3:Array<Item?> = arrayOf(
        Weapon(name = "Sword", type = "Weapon", drawable =  R.drawable.basicattack, levelRq = 1, quality = 0,charClass = 3, description = "The most sold weapon on black market", slot = 0)
        ,Weapon(name = "Shield", type = "Weapon", drawable = R.drawable.shield, levelRq = 1, quality = 0,charClass = 3, description = "Blocks 80% of next enemy attack\nYou can't use it as a boat anymore after all this", slot = 1)
        ,Wearable(name = "Belt", type ="Wearable", drawable = R.drawable.belt, levelRq = 1, quality = 0,charClass = 3, description = "I can't breath", slot = 4)
        ,Wearable(name = "Overall", type ="Wearable", drawable = R.drawable.overall, levelRq = 1, quality = 0,charClass = 3, description = "What is that smell?", slot = 5)
        ,Wearable(name = "Boots", type ="Wearable", drawable = R.drawable.boots, levelRq = 1, quality = 0,charClass = 3, description = "Can't carry it anymore", slot = 6)
        ,Wearable(name = "Trousers", type ="Wearable", drawable = R.drawable.trousers, levelRq = 1, quality = 0,charClass = 3, description = "Tight not high", slot = 7)
        ,Wearable(name = "Chestplate", type ="Wearable", drawable = R.drawable.chestplate, levelRq = 1, quality = 0,charClass = 3, description = "Chestplate protects!", slot = 8)
        ,Wearable(name = "Helmet", type ="Wearable", drawable = R.drawable.helmet, levelRq = 1, quality = 0,charClass = 3, description = "This doesn't make you any more clever", slot = 9)
)

val itemsClass4:Array<Item?> = arrayOf(
        Weapon(name = "Sword", type = "Weapon", drawable =  R.drawable.basicattack, levelRq = 1, quality = 0,charClass = 4, description = "The most sold weapon on black market", slot = 0)
        ,Weapon(name = "Shield", type = "Weapon", drawable = R.drawable.shield, levelRq = 1, quality = 0,charClass = 4, description = "Blocks 80% of next enemy attack\nYou can't use it as a boat anymore after all this", slot = 1)
        ,Wearable(name = "Belt", type ="Wearable", drawable = R.drawable.belt, levelRq = 1, quality = 0,charClass = 4, description = "I can't breath", slot = 4)
        ,Wearable(name = "Overall", type ="Wearable", drawable = R.drawable.overall, levelRq = 1, quality = 0,charClass = 4, description = "What is that smell?", slot = 5)
        ,Wearable(name = "Boots", type ="Wearable", drawable = R.drawable.boots, levelRq = 1, quality = 0,charClass = 4, description = "Can't carry it anymore", slot = 6)
        ,Wearable(name = "Trousers", type ="Wearable", drawable = R.drawable.trousers, levelRq = 1, quality = 0,charClass = 4, description = "Tight not high", slot = 7)
        ,Wearable(name = "Chestplate", type ="Wearable", drawable = R.drawable.chestplate, levelRq = 1, quality = 0,charClass = 4, description = "Chestplate protects!", slot = 8)
        ,Wearable(name = "Helmet", type ="Wearable", drawable = R.drawable.helmet, levelRq = 1, quality = 0,charClass = 4, description = "This doesn't make you any more clever", slot = 9)
)

val itemsClass5:Array<Item?> = arrayOf(
        Weapon(name = "Sword", type = "Weapon", drawable =  R.drawable.basicattack, levelRq = 1, quality = 0,charClass = 5, description = "The most sold weapon on black market", slot = 0)
        ,Weapon(name = "Shield", type = "Weapon", drawable = R.drawable.shield, levelRq = 1, quality = 0,charClass = 5, description = "Blocks 80% of next enemy attack\nYou can't use it as a boat anymore after all this", slot = 1)
        ,Wearable(name = "Belt", type ="Wearable", drawable = R.drawable.belt, levelRq = 1, quality = 0,charClass = 5, description = "I can't breath", slot = 4)
        ,Wearable(name = "Overall", type ="Wearable", drawable = R.drawable.overall, levelRq = 1, quality = 0,charClass = 5, description = "What is that smell?", slot = 5)
        ,Wearable(name = "Boots", type ="Wearable", drawable = R.drawable.boots, levelRq = 1, quality = 0,charClass = 5, description = "Can't carry it anymore", slot = 6)
        ,Wearable(name = "Trousers", type ="Wearable", drawable = R.drawable.trousers, levelRq = 1, quality = 0,charClass = 5, description = "Tight not high", slot = 7)
        ,Wearable(name = "Chestplate", type ="Wearable", drawable = R.drawable.chestplate, levelRq = 1, quality = 0,charClass = 5, description = "Chestplate protects!", slot = 8)
        ,Wearable(name = "Helmet", type ="Wearable", drawable = R.drawable.helmet, levelRq = 1, quality = 0,charClass = 5, description = "This doesn't make you any more clever", slot = 9)
)

val itemsClass6:Array<Item?> = arrayOf(
        Weapon(name = "Sword", type = "Weapon", drawable =  R.drawable.basicattack, levelRq = 1, quality = 0,charClass = 6, description = "The most sold weapon on black market", slot = 0)
        ,Weapon(name = "Shield", type = "Weapon", drawable = R.drawable.shield, levelRq = 1, quality = 0,charClass = 6, description = "Blocks 80% of next enemy attack\nYou can't use it as a boat anymore after all this", slot = 1)
        ,Wearable(name = "Belt", type ="Wearable", drawable = R.drawable.belt, levelRq = 1, quality = 0,charClass = 6, description = "I can't breath", slot = 4)
        ,Wearable(name = "Overall", type ="Wearable", drawable = R.drawable.overall, levelRq = 1, quality = 0,charClass = 6, description = "What is that smell?", slot = 5)
        ,Wearable(name = "Boots", type ="Wearable", drawable = R.drawable.boots, levelRq = 1, quality = 0,charClass = 6, description = "Can't carry it anymore", slot = 6)
        ,Wearable(name = "Trousers", type ="Wearable", drawable = R.drawable.trousers, levelRq = 1, quality = 0,charClass = 6, description = "Tight not high", slot = 7)
        ,Wearable(name = "Chestplate", type ="Wearable", drawable = R.drawable.chestplate, levelRq = 1, quality = 0,charClass = 6, description = "Chestplate protects!", slot = 8)
        ,Wearable(name = "Helmet", type ="Wearable", drawable = R.drawable.helmet, levelRq = 1, quality = 0,charClass = 6, description = "This doesn't make you any more clever", slot = 9)
)

val itemsClass7:Array<Item?> = arrayOf(
        Weapon(name = "Sword", type = "Weapon", drawable =  R.drawable.basicattack, levelRq = 1, quality = 0,charClass = 7, description = "The most sold weapon on black market", slot = 0)
        ,Weapon(name = "Shield", type = "Weapon", drawable = R.drawable.shield, levelRq = 1, quality = 0,charClass = 7, description = "Blocks 80% of next enemy attack\nYou can't use it as a boat anymore after all this", slot = 1)
        ,Wearable(name = "Belt", type ="Wearable", drawable = R.drawable.belt, levelRq = 1, quality = 0,charClass = 7, description = "I can't breath", slot = 4)
        ,Wearable(name = "Overall", type ="Wearable", drawable = R.drawable.overall, levelRq = 1, quality = 0,charClass = 7, description = "What is that smell?", slot = 5)
        ,Wearable(name = "Boots", type ="Wearable", drawable = R.drawable.boots, levelRq = 1, quality = 0,charClass = 7, description = "Can't carry it anymore", slot = 6)
        ,Wearable(name = "Trousers", type ="Wearable", drawable = R.drawable.trousers, levelRq = 1, quality = 0,charClass = 7, description = "Tight not high", slot = 7)
        ,Wearable(name = "Chestplate", type ="Wearable", drawable = R.drawable.chestplate, levelRq = 1, quality = 0,charClass = 7, description = "Chestplate protects!", slot = 8)
        ,Wearable(name = "Helmet", type ="Wearable", drawable = R.drawable.helmet, levelRq = 1, quality = 0,charClass = 7, description = "This doesn't make you any more clever", slot = 9)
)

val itemsClass8:Array<Item?> = arrayOf(
        Weapon(name = "Sword", type = "Weapon", drawable =  R.drawable.basicattack, levelRq = 1, quality = 0,charClass = 8, description = "The most sold weapon on black market", slot = 0)
        ,Weapon(name = "Shield", type = "Weapon", drawable = R.drawable.shield, levelRq = 1, quality = 0,charClass = 8, description = "Blocks 80% of next enemy attack\nYou can't use it as a boat anymore after all this", slot = 1)
        ,Wearable(name = "Belt", type ="Wearable", drawable = R.drawable.belt, levelRq = 1, quality = 0,charClass = 8, description = "I can't breath", slot = 4)
        ,Wearable(name = "Overall", type ="Wearable", drawable = R.drawable.overall, levelRq = 1, quality = 0,charClass = 8, description = "What is that smell?", slot = 5)
        ,Wearable(name = "Boots", type ="Wearable", drawable = R.drawable.boots, levelRq = 1, quality = 0,charClass = 8, description = "Can't carry it anymore", slot = 6)
        ,Wearable(name = "Trousers", type ="Wearable", drawable = R.drawable.trousers, levelRq = 1, quality = 0,charClass = 8, description = "Tight not high", slot = 7)
        ,Wearable(name = "Chestplate", type ="Wearable", drawable = R.drawable.chestplate, levelRq = 1, quality = 0,charClass = 8, description = "Chestplate protects!", slot = 8)
        ,Wearable(name = "Helmet", type ="Wearable", drawable = R.drawable.helmet, levelRq = 1, quality = 0,charClass = 8, description = "This doesn't make you any more clever", slot = 9)
)