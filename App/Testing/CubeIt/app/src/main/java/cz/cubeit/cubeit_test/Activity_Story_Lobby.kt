...
class Activity_Story_Lobby: SystemFlow.GameActivity(R.layout.activity_story_lobby, ActivityType.StoryLobby, true, R.id.imageViewStoryLobbyBg, R.color.colorSecondary, hasSwipeMenu = false, hasSwipeDown = false){
    ...
    fun spacing(pointX: Coordinates, pointY: Coordinates): Float {
        val x = pointX.x - pointY.x
        val y = pointX.y - pointY.y
        val s = x * x + y * y
        return sqrt(s)
    }
                    ...
                    when {
                        ((initialViewX + (distanceX / 1.3)) + (dm.heightPixels * 0.1)) < dm.widthPixels * 0.5 -> {
                            imageViewStoryLobbyShadowLeft.apply {
                                layoutParams.width = min(dm.widthPixels * 0.75, initialShadowWidthLeft + spacing(
                                        Coordinates(initialViewX, initialViewY),
                                        Coordinates(initialViewX + distanceX, initialViewY + distanceY)
                                ) * 1.4).toInt()
                            ...
                            imageViewStoryLobbyShadowRight.apply {
                                layoutParams.width = min(dm.widthPixels * 0.75, initialShadowWidthRight + spacing(
                                        Coordinates(initialViewX, initialViewY),
                                        Coordinates(initialViewX + distanceX, initialViewY + distanceY)
                                ) * 1.4).toInt()
