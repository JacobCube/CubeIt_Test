...
class Fragment_Adventure : Fragment() {         //TODO automatické generování komponentů na mapu
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        if(Data.player.currentSurfaces.getOrNull(surfaceIndex)?.boss != null && Data.player.currentSurfaces.getOrNull(surfaceIndex)?.boss!!.isActive()){
            ...
            viewPop.imageViewAdventure.setImageBitmap(Data.player.currentSurfaces.getOrNull(surfaceIndex)?.boss?.bitmap)
            viewPop.imageViewAdventure.background = BitmapDrawable(resources, Data.player.currentSurfaces.getOrNull(surfaceIndex)?.boss?.bitmapBg)
            ...
            if(Data.player.currentSurfaces.getOrNull(surfaceIndex)?.boss?.reward?.item != null){
                ...
                viewPop.imageViewAdventure2.setImageBitmap(Data.player.currentSurfaces.getOrNull(surfaceIndex)?.boss!!.reward.item?.bitmap)
                viewPop.imageViewAdventure2.setBackgroundResource(Data.player.currentSurfaces.getOrNull(surfaceIndex)?.boss!!.reward.item?.getBackground() ?: 0)

                val tempActivity = activity as SystemFlow.GameActivity
                viewPop.imageViewAdventure2.setUpOnHoldDecorPop(tempActivity, Data.player.currentSurfaces.getOrNull(surfaceIndex)?.boss!!.reward.item!!, false)
            }else {
