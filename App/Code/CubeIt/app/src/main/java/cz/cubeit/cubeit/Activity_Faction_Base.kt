...
class Activity_Faction_Base: SystemFlow.GameActivity(R.layout.activity_faction_base, ActivityType.Faction, true, R.id.imageViewFactionBg, hasSwipeMenu = false){           //arguments - id: String
            ...
            if(viewPagerFactionTemp.currentItem == 2 && Data.player.faction != null){   //TODO porovnání provedených změn v pozadí
                ...
                if(SystemFlow.factionChange){
                    ...
                    buttonYes.setOnClickListener {
                        ...
                        for(i in inviteList){
                            Data.player.writeInbox(i, InboxMessage(
                                    status = MessageStatus.Faction,
                                    receiver = i,
                                    sender = Data.player.username,
                                    subject = "${Data.player.faction!!.name} invited you.",
                                    content = editTextFactionEditInvitationMsg.text.toString(),
                                    isInvitation1 = true,
                                    invitation = Invitation(Data.player.username, " invited you to faction ", Data.player.faction!!.name, InvitationType.faction, Data.player.factionID ?: "", Data.player.factionName!!)
                            ))
                        }
    ...
    private class ViewPagerFactionOverview internal constructor(fm: FragmentManager, private val fractionID: String?) : FragmentPagerAdapter(fm, BEHAVIOR_RESUME_ONLY_CURRENT_FRAGMENT){
