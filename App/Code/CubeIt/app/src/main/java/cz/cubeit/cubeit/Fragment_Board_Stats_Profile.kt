...
class Fragment_Board_Stats_Profile : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        view.profile_description.setHTMLText(playerProfile.description)
        view.textViewProfileXp.setHTMLText(GameFlow.experienceScaleFormatString(playerProfile.experience, playerProfile))
        view.textViewProfileLevel.setHTMLText(playerProfile.level)
        view.progressBarProfileXp.max = playerProfile.getRequiredXp()
        ...
        view.imageViewProfileStatsAlly.setOnClickListener {
            view.imageViewProfileStatsAlly.visibility = View.GONE
            Data.player.requestSocialAlly(playerProfile.username, playerProfile.profilePicId, view.context)
            view.imageViewBoardProfieStatsBlock.visibility = View.GONE
        }
        ...
        view.profile_stats_fight.setOnClickListener {
            if(playerProfile.username != Data.player.username){
                ...
                var canceled = false
                val loading = SystemFlow.createLoading(activity as SystemFlow.GameActivity, startAutomatically = true, cancelable = true, listener = View.OnClickListener { canceled = true })
                playerProfile.loadPlayer().addOnCompleteListener {
                    loading.cancel()
                    if(it.isSuccessful && !canceled){
