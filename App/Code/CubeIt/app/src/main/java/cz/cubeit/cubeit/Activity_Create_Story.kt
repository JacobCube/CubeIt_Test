...
class Activity_Create_Story: SystemFlow.GameActivity(contentLayoutId = R.layout.activity_create_story, activityType = ActivityType.CreateStory, hasMenu = false, hasSwipeDown = false){
    ...
    private var currentPropertiesOptions: FrameLayout? = null
    ...
    private var slideActionMode = false
    ...
    private var componentsManagerOpened = false

    var layoutWidth: Int = 1
        get() = if(maximized) dm.widthPixels else (dm.widthPixels.toDouble() * 0.78).toInt()
    var layoutHeight: Int = 1
        get() = if(maximized) dm.heightPixels else (dm.heightPixels.toDouble() * 0.78).toInt()
    ...
    private fun updateLayoutImage(){
        layoutCreateStoryField.post {
            if(layoutCreateStoryField.height > 0 && layoutCreateStoryField.childCount == storyQuest.slides[currentSlideIndex].components.size){
                ...
                storyQuest.overviewBitmapIds.removeAll { it == (storyQuest.slides[currentSlideIndex].id) }
                storyQuest.overviewBitmapIds.add(storyQuest.slides[currentSlideIndex].id)
    ...
    fun attachComponent(view: View){
        ...
        Data.FrameworkData.saveDraft(storyQuest, view.context)
    ...
    private var clearFieldActionModeHandler = Handler()
    override fun dispatchTouchEvent(ev: MotionEvent): Boolean {
        ...
        if(!slideActionMode && currentPropertiesOptions != null && !propertiesOptions.contains(ev.rawX.toInt(), ev.rawY.toInt()) && !propertiesOptions.contains(lastRecognizedPointer.x.toInt(), lastRecognizedPointer.y.toInt()) && ev.action == MotionEvent.ACTION_UP){
            removeCurrentPropertiesOptions(removeMarkers = false, reIndex = false)
            Log.d("dispatchTouchEvent", "currentPropertiesOptions is not null")
        }
        if(!slideActionMode && currentPropertiesOptions == null && !currentComponent.contains(ev.rawX.toInt(), ev.rawY.toInt()) && !currentComponent.contains(lastRecognizedPointer.x.toInt(), lastRecognizedPointer.y.toInt()) && ev.action == MotionEvent.ACTION_UP && !componentManager.contains(ev.rawX.toInt(), ev.rawY.toInt())){
            removeCurrentPropertiesOptions(removeMarkers = false, reIndex = false)
            draggedComponent = null
            Log.d("dispatchTouchEvent", "currentPropertiesOptions is null")
        }
                ...
                if(abs(originalDistanceX) >= swipeSlideThreshold && abs(originalDistanceX) > abs(originalDistanceY)){
                    if(swipeSlideValid                                  //delay still valid
                        ...
                        if(originalDistanceX > 0){
                            if(slideActionMode){
                                selectSlide(max(currentSlideIndex, 0))
                            }else selectSlide(max(currentSlideIndex - 1, 0))
                            hideLastSlideActions()
                        }else {
                            if(currentSlideIndex + 1 == storyQuest.slides.size){
                                clearFieldActionModeHandler.removeCallbacksAndMessages(null)
                                clearFieldActionModeHandler.postDelayed({
                                    for(i in storyQuest.slides[currentSlideIndex].components){
                                        i.view = null
                                    }
                                    layoutCreateStoryField.removeAllViews()
                                    imageViewCreateStoryAddSlide2.visibility = View.VISIBLE
                                    imageViewCreateStoryUpload.visibility = View.VISIBLE
                                    slideActionMode = true
                                }, 200)
                            }else {
                                selectSlide(min(currentSlideIndex + 1, storyQuest.slides.size - 1))
                                hideLastSlideActions()
                            }
                        }
                    }
    ...
    private fun selectSlide(position: Int){
        changingSlide = true
        tabLayoutCreateStorySlides.getTabAt(position)?.select()
        changingSlide = false
    }

    private fun hideLastSlideActions(){
        slideActionMode = false
        if(imageViewCreateStoryAddSlide2.visibility == View.VISIBLE){
            relayout()

            imageViewCreateStoryAddSlide2.visibility = View.GONE
            imageViewCreateStoryUpload.visibility = View.GONE
        }
    }
    ...
    private fun createNewSlide(){
        if(storyQuest.slides.size <= 30){
            ...
            Snackbar.make(imageViewCreateStoryAddSlide, "What are you even trying to achieve? We don't support more than 30 slides yet. Open manager to remove some of your slides.", Snackbar.LENGTH_LONG).show()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        parent.post {
            ...
            imageViewCreateStoryAddSlide2.apply {
                layoutParams.width = (dm.widthPixels * 0.14).toInt()
                layoutParams.height = (dm.widthPixels * 0.14).toInt()
                setOnClickListener {
                    createNewSlide()
                }
            }
            imageViewCreateStoryUpload.apply {
                layoutParams.width = (dm.widthPixels * 0.14).toInt()
                layoutParams.height = (dm.widthPixels * 0.14).toInt()
                setOnClickListener {
                    val intent = Intent(this@Activity_Create_Story, Activity_Create_Story_Publish()::class.java)
            ...
            imageViewOpenComponentsManager?.apply {
                ...
                setOnClickListener {
                    disableManagerHandler.removeCallbacksAndMessages(null)
                    imageViewOpenComponentsManager?.isEnabled = false
                    disableManagerHandler.postDelayed({
            ...
            recyclerViewComponentsManager?.apply {
                ...
                y = 0f
                elevation = 1f
                setBackgroundResource(R.drawable.bg_basic_brown3_border)
                setPadding(6, 6, 6, 6)
            }
    ...
    private class CreateStoryOverviewSecondary(var bitmapIds: MutableList<String>, var ratioDescriptions: MutableList<String>, val activity: Activity_Create_Story, val template: SystemFlow.FrameworkComponentTemplate) :
        ...
        override fun getItemCount() = bitmapIds.size
        ...
        override fun onBindViewHolder(viewHolder: CategoryViewHolder, position: Int) {
            viewHolder.imageViewIcon.setImageBitmap(Data.downloadedBitmaps[bitmapIds[position]])
            ...
            viewHolder.imageViewBg.setOnLongClickListener {
                ...
                val item = ClipData.Item(component.bitmapId)
                val dragData = ClipData(
                        "storyComponent",
                        arrayOf(component.bitmapId),
                        item)
                activity.draggedComponent = component
                SystemFlow.vibrateAsError(activity)

                val myShadow = SystemFlow.StoryDragListener(null, component.realWidth, component.realHeight, component.rotationAngle, BitmapDrawable(activity.resources , Data.downloadedBitmaps[bitmapIds[position]]))
    ...
    private class CreateStoryOverview(var components: MutableList<SystemFlow.FrameworkComponentTemplate>, val activity: Activity_Create_Story, val layoutField: ConstraintLayout?) :
        ...
        override fun onBindViewHolder(viewHolder: CategoryViewHolder, position: Int) {
            viewHolder.imageViewIcon.setImageBitmap(components[position].bitmapIcon)
            ...
            for(i in components[position].bitmaps){
                ...
                descriptions.add("base resolution: ${((i?.width?.toDouble() ?: 1.0) / (i?.height?.toDouble() ?: 1.0) * 10).toInt()}:10")
            }
            ...
            viewHolder.imageViewBg.setOnClickListener {
                ...
                if(components[position].bitmaps.isNotEmpty()){
    ...
    private class CreateStoryComponentsManager(val parent: Activity_Create_Story) :
        ...
        override fun onBindViewHolder(viewHolder: ItemViewHolder, position: Int) {
            ...
            viewHolder.imageViewComponent.setImageBitmap(currentComponent.bitmap)
            ...
            viewHolder.textViewDsc.setHTMLText("${currentComponent.innerIndex}<br/>width: ${currentComponent.width.toInt()}%<br/>height: ${currentComponent.height.toInt()}%")
        }
    }
            ...
            DragEvent.ACTION_DROP -> {
                if(draggedComponent != null) {
                    ...
                    with(draggedComponent!!){
                        realCoordinates.x = max(0f, min((layoutWidth - (draggedComponent?.realWidth ?: 0)).toFloat(), leftCoords.x))
                        realCoordinates.y = max(0f, min((layoutHeight - (draggedComponent?.realHeight ?: 0)).toFloat(), locationCoords.y - ((draggedComponent?.realHeight ?: 0) / 2)))
