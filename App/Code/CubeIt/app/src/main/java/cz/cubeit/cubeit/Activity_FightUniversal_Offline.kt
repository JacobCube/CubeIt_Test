...
class ActivityFightUniversalOffline: SystemFlow.GameActivity(R.layout.activity_fight_universal_offline, ActivityType.FightUniversalOffline, false){
    ...
    private var myCustomOptions: SystemFlow.OverlayOptionsMenu? = null
    ...
    override fun onBackPressed() {
        if(myCustomOptions != null){
            ...
            if(myRoundTimer > 0){
                ...
                myCustomOptions?.remove()
                myCustomOptions = null
            }
        }
    ...
    private fun endFight(){
        Log.d("storyMode", storyMode.toString())
        if(storyMode && !universalOffline.fightCompleted){
            Handler().postDelayed({
                setResult(if(universalOffline.winnerTeam == FightSystem.FighterType.Ally) CODE_FIGHT_WON else CODE_FIGHT_LOST)
                finish()
            }, 600)
        ...
        if(!universalOffline.fightCompleted){
            ...
            Log.d("endFight", "called, winnerTeam: ${universalOffline.winnerTeam}")
            universalOffline.fightCompleted = true
        }
    }
    ...
    private fun applyCompleteRound(parent: ActivityFightUniversalOffline){
        ...
        myRoundTimer = 0
        var allySpellAnimation: ValueAnimator? = null
        var enemySpellAnimation: ValueAnimator? = null
        ...
        parent.runOnUiThread {
            imageViewSpellAlly.apply {
                ...
                setImageBitmap(universalOffline.currentAlly?.chosenSpell?.bitmap)
            }
            imageViewSpellEnemy.apply {
                ...
                layoutParams.width = fragmentSpellFight0?.height ?: 0
                visibility = View.GONE
                setImageBitmap(universalOffline.currentEnemy?.chosenSpell?.bitmap)
            }
        }
        ...
        if(allySpell != null){
            allySpellAnimation = parent.imageViewSpellAlly.animateSpell(allyValid,
                    when {
                        allySpell.block != 1.0 || allySpell.defensive -> {
                            SpellAnimationType.defense
                        }
            ...
            allySpellAnimation.addListener(object : Animator.AnimatorListener {
                ...
                override fun onAnimationEnd(animation: Animator) {
                    ...
                    if(universalOffline.prepareRound(reward)){
                        ...
                        if(!allySpell.defensive) universalOffline.currentAlly?.chosenSpell = null
                        //start only if ally started, so enemy should follow AND enemy exists AND spells aren't both defensive (spells start at the same time at that point)
                        if(universalOffline.currentRound?.whoStarts == FightSystem.FighterType.Ally && universalOffline.currentEnemy != null && !(allySpell.defensive && enemySpell?.defensive == true)) enemySpellAnimation?.start()
                    }
                }
        ...
        if(enemySpell != null){
            enemySpellAnimation = parent.imageViewSpellEnemy.animateSpell(enemyValid,
                    when {
                        enemySpell.block != 1.0 || enemySpell.defensive -> {
                            SpellAnimationType.defense
                        }
                        else -> SpellAnimationType.normal
                    }
            ...
            enemySpellAnimation.addListener(object : Animator.AnimatorListener {
                ...
                override fun onAnimationEnd(animation: Animator) {
                    ...
                    if(universalOffline.prepareRound(reward)){
                        ...
                        if(!enemySpell.defensive) universalOffline.currentEnemy?.chosenSpell = null
                        //start only if ally started, so enemy should follow AND enemy exists AND spells aren't both defensive (spells start at the same time at that point)
                        if(universalOffline.currentRound?.whoStarts == FightSystem.FighterType.Enemy && universalOffline.currentAlly != null && !(allySpell?.defensive == true && enemySpell.defensive)) allySpellAnimation?.start()
                    }
                }
        ...
        handlerStartRound.postDelayed({
            when{
                //both fighters are using defensive spell, animate them at the same time
                allySpell?.defensive == true && enemySpell?.defensive == true && universalOffline.currentAlly != null && universalOffline.currentEnemy != null -> {
                    parent.imageViewSpellAlly.post {
                        allySpellAnimation?.start()
                        enemySpellAnimation?.start()
                    }
                }
                //enemy fighter used defensive spell, prioritize him, to make effect of defensive spell valid
                enemySpell?.defensive == true && universalOffline.currentEnemy != null -> {
                    universalOffline.currentRound?.whoStarts = FightSystem.FighterType.Enemy
                    parent.imageViewSpellAlly.post {
                        enemySpellAnimation?.start()
                    }
                }
                //ally fighter used defensive spell, prioritize him, to make effect of defensive spell valid
                allySpell?.defensive == true && universalOffline.currentAlly != null -> {
                    universalOffline.currentRound?.whoStarts = FightSystem.FighterType.Ally
                    parent.imageViewSpellAlly.post {
                        allySpellAnimation?.start()
                    }
                }
    ...
    private fun onMyRoundEnd(parent: ActivityFightUniversalOffline){
        ...
        spells.addAll(mutableListOf(Data.player.defaultAttackSpell, Data.player.defaultDefenseSpell))
        ...
        return (object : TimerTask() {
            override fun run() {
                ...
                if(myRoundTimer > 1){
                    ...
                    universalOffline.allies.find { it.name == Data.player.username }?.chosenSpell = Data.player.defaultDefenseSpell
                    universalOffline.currentRound?.allySpell = Data.player.defaultDefenseSpell
                    onMyRoundEnd(parent)
        ...
        this.setOnTouchListener(object: Class_DragOutTouchListener(this, false, true, imageViewShadow, parent){
            ...
            override fun solidHold(x: Float, y: Float) {
                ...
                if(!windowPop.isShowing && !viewPinned){
                    ...
                    viewP.imageViewPopUpInfoItem.setImageBitmap(chosenSpell?.bitmap)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        universalOffline.isStoryFight = intent?.extras?.getBoolean("isStoryFight", false) ?: false
        storyMode = universalOffline.isStoryFight
        ...
        imageViewUniversalFightOfflinePause.setOnClickListener {
            if(myRoundTimer > 0 && choosingSpellTimer != null){
                if(!timerPaused){
                    myCustomOptions = SystemFlow.OverlayOptionsMenu(this)
                    Data.downloadedBitmaps["icon_exit"] = BitmapFactory.decodeResource(resources, R.drawable.icon_exit, opts)
                    Data.downloadedBitmaps["ic_dialog_info"] = BitmapFactory.decodeResource(resources, android.R.drawable.ic_dialog_info, opts)
                    Data.downloadedBitmaps["close_image"] = BitmapFactory.decodeResource(resources, R.drawable.close_image, opts)
                    myCustomOptions?.create(mutableListOf(
                            SystemFlow.OverlayOptionsMenuItem("icon_exit", View.OnClickListener {
                                ...
                                info.setHTMLText("Do you really want to leave this fight?")
                                window.isOutsideTouchable = false
                                window.isFocusable = true
                                window.setBackgroundDrawable(ColorDrawable(Color.TRANSPARENT))
                                buttonYes.setOnClickListener {
                                    universalOffline.endFight(FightSystem.FighterType.Enemy, null)
                                    myCustomOptions?.remove()
                                    myCustomOptions = null
                                    endFight()
                                    window.dismiss()
                            ...
                            SystemFlow.OverlayOptionsMenuItem("ic_dialog_info", View.OnClickListener {
                                Toast.makeText(this, "Cannot help yet, sorry.", Toast.LENGTH_SHORT).show()
                                cancelPauseListener.onClick(it)
                            }, "info"),
                            SystemFlow.OverlayOptionsMenuItem("close_image", cancelPauseListener, "cancel")
                    ), false, SystemFlow.InitDynamicAnimation.SCALE_FROM_SMALL)
    ...
    class FightOfflineAllyList(
        ...
        override fun onBindViewHolder(viewHolder: CategoryViewHolder, position: Int) {
            ...
            viewHolder.viewGroup.visibility = if(!allies[position].dead){
                ...
                viewHolder.imageViewChar.setImageBitmap(allies[position].sourceNPC?.charClass?.bitmapIcon ?: allies[position].sourcePlayer?.charClass?.bitmapIcon ?: allies[position].sourceBoss?.charClass?.bitmapIcon)
    ...
    class FightOfflineEnemyList(
        ...
        override fun onBindViewHolder(viewHolder: CategoryViewHolder, position: Int) {
            ...
            viewHolder.viewGroup.visibility = if(!enemies[position].dead){
                ...
                viewHolder.imageViewChar.setImageBitmap(enemies[position].sourceNPC?.charClass?.bitmapIcon ?: enemies[position].sourcePlayer?.charClass?.bitmapIcon ?: enemies[position].sourceBoss?.charClass?.bitmapIcon)
