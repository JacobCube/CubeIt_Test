...
object FightSystem{
    ...
    class Fighter(
        ...
        init {
            when {
                ...
                sourceNPC != null -> {
                    ...
                    name = sourceNPC?.name ?: ""
                    drawable = sourceNPC?.drawable ?: sourceNPC?.charClass?.drawable ?: 0
                ...
                sourceBoss != null -> {
                    ...
                    drawable = sourceBoss?.drawable ?: sourceBoss?.charClass?.drawable ?: 0
                    drawableBg = sourceBoss?.bgDrawable ?: 0
        ...
        fun originalDefaultSpell(): Spell? {
            return sourcePlayer?.learnedSpells?.firstOrNull() ?: sourceNPC?.allowedSpells?.firstOrNull() ?: sourceBoss?.allowedSpells?.firstOrNull()
        }
    ...
    class UniversalFightOffline(
        ...
        private fun endFight(team: FighterType, reward: Reward?){
            ...
            when{
                isTestFight -> return
                bossFightSurface > -1 -> {
                    Log.d("endfight", "bossFightSurface $bossFightSurface")
                    Data.player.currentSurfaces[bossFightSurface].boss?.detach()
                    return
                }
                !isFameFight -> {
                    Log.d("endfight", "isFameFight")
