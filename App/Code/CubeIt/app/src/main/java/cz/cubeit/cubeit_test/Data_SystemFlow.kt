...
object SystemFlow {
        ...
        override fun dispatchTouchEvent(ev: MotionEvent): Boolean {
            ...
            when (ev.action) {
                MotionEvent.ACTION_DOWN -> {
                    ...
                    eventType = when {
                        ev.rawY <= dm.heightPixels * 0.225 && ev.rawX.between((dm.widthPixels * 0.25).toFloat(), (dm.widthPixels * 0.75).toFloat()) -> {
                        ...
                        ev.rawY >= dm.heightPixels * 0.7 -> {
                            ...
                            frameLayoutMenuBar?.bringToFront()
                            when {
                                hasMenu || (frameLayoutMenuBar?.y ?: 9999f) < dm.heightPixels * 0.9 -> {
                                    2
                                }
                                ev.rawY <= dm.heightPixels * 0.9 -> {
                                    2
                                }
                                else -> 0
                            }
        ...
        override fun onDestroy() {
            ...
            for(i in 0..parent.childCount){
                if(parent.getChildAt(i) is ImageView){
                    Log.d("GameActivity_recycler", "recycler view of index: $i")
    ...
    fun showSocials(activity: GameActivity): FrameLayout {
        return attachNewFragment(
                ...
                (activity.dm.widthPixels * 0.6).toInt(),
    ...
    class OverlayOptionsMenu(
        ...
        var background: ImageView = ImageView(activity)
        var recyclerView: RecyclerView = RecyclerView(activity)

        fun create(activity: GameActivity, options: Pair<Int, View.OnClickListener>, closeable: Boolean = true) {
            remove()
            val parent = activity.window.decorView.findViewById<ViewGroup>(android.R.id.content)
            ...
            recyclerView.apply {
                ...
                isClickable = true
                isFocusable = true
                x = (activity.dm.widthPixels * 0.15).toFloat()
                y = (activity.dm.heightPixels * 0.1).toFloat()
                layoutParams.width = (activity.dm.widthPixels * 0.7).toInt()
                layoutParams.height = (activity.dm.heightPixels * 0.8).toInt()
    ...
    data class FrameworkComponent(
        ...
        fun getCoordinatesFromReal(activity: GameActivity) {
            ...
            coordinates.x = (realCoordinates.x / layoutWidth * 100)
            coordinates.y = (realCoordinates.y / layoutHeight * 100)
        }

        fun resolveSizeByDrawable(activity: GameActivity) {
            val layoutWidth = (activity as? Activity_Create_Story)?.layoutWidth ?: currentGameActivity?.dm?.widthPixels ?: 1
            val layoutHeight = (activity as? Activity_Create_Story)?.layoutHeight ?: currentGameActivity?.dm?.widthPixels ?: 1
