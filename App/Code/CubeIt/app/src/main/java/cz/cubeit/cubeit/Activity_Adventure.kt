...
class ActivityAdventure : SystemFlow.GameActivity(R.layout.activity_adventure, ActivityType.Adventure, true) {
    ...
    var overviewList: MutableList<Quest>? = null
    ...
    var viewPagerAdventureSurfacesTemp: ViewPager? = null

    private var iconSideQuestsAnim = ValueAnimator()

    override fun dispatchTouchEvent(ev: MotionEvent): Boolean {
        val overviewRect = Rect()
        val overviewButtonRect = Rect()
        frameLayoutAdventureOverview?.getGlobalVisibleRect(overviewRect)
        overviewQuestIconTemp?.getGlobalVisibleRect(overviewButtonRect)

        if(overViewOpened && !overviewRect.contains(ev.rawX.toInt(), ev.rawY.toInt())
                && !overviewRect.contains(lastRecognizedPointer.x.toInt(), lastRecognizedPointer.y.toInt())
                && ev.action == MotionEvent.ACTION_UP
                && !overviewButtonRect.contains(ev.rawX.toInt(), ev.rawY.toInt())){
            overviewQuestIcon.performClick()
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(Data.player.currentSurfaces.any { it.quests.isEmpty() }){
            Handler().postDelayed({
        ...
        overviewQuestIcon.setOnClickListener {
            ...
            overViewOpened = if(!overViewOpened){
                iconSideQuestsAnim = ValueAnimator.ofFloat(overviewQuestIcon.x, (dm.widthPixels - (overviewQuestIcon.width + frameLayoutAdventureOverview.width)).toFloat()).apply{
                    duration = 800
                    addUpdateListener {
                ...
                iconSideQuestsAnim = ValueAnimator.ofFloat(overviewQuestIcon.x + overviewQuestIcon.width.toFloat(), dm.widthPixels.toFloat()).apply{
                    duration = 800
                    addUpdateListener {
                        overviewQuestIcon.x = it.animatedValue as Float - overviewQuestIcon.width.toFloat()
                        frameLayoutAdventureOverview.x = it.animatedValue as Float + 8f
                    }
                    addListener(object : Animator.AnimatorListener {
                            override fun onAnimationStart(animation: Animator) {
                                overviewQuestIcon.isEnabled = false
        ...
        viewPagerAdventureSurfaces?.adapter = ViewPagerAdapterAdventure(supportFragmentManager)
        viewPagerAdventureSurfaces?.offscreenPageLimit = 6
        viewPagerAdventureSurfacesTemp = viewPagerAdventureSurfaces
    }
    ...
    fun onClickQuest(index: Int, surface: Int, anchor: View){
        ...
        val imageViewAdventure: ImageView = viewPop.imageViewPopAdventureQuestItem
        ...
        val item = quest.reward.item ?: Item()
        imageViewAdventure.setUpOnHoldDecorPop(this, true, item.getStatsCompare(), item.getBackground(), item.bitmap)
    ...
    fun onClickQuestOverview(surface:Int, index:Int, context: Context, questA: Quest? = null, questIn: Quest? = null, progressAdventureQuest: ProgressBar, textViewQuestProgress: CustomTextView, viewPopQuest: View, fragmentOverview: Fragment?, usedActivity: SystemFlow.GameActivity){
        ...
        val imageViewAdventure: ImageView = viewPopQuest.imageViewPopAdventureQuestItem
        ...
        val item = quest.reward.item ?: Item()
        imageViewAdventure.setUpOnHoldDecorPop(usedActivity, true, item.getStatsCompare(), item.getBackground(), item.bitmap)
    ...
    class ViewPagerAdapterAdventure internal constructor(fm: FragmentManager) : FragmentPagerAdapter(fm, BEHAVIOR_RESUME_ONLY_CURRENT_FRAGMENT){
        val arrayFragments = arrayOfNulls<Fragment_Adventure>(6)

        override fun getItem(position: Int): Fragment {
            arrayFragments[position] = when(position) {
            ...
            return arrayFragments[position] ?: Fragment_Adventure.newInstance(R.drawable.map0, 0)
        }
