...
class FragmentDefense : SystemFlow.GameFragment(R.layout.fragment_defense, R.id.layoutFragmentDefense){
    ...
    override fun onStop() {
        ...
        while(true){            //corrects energy usage by the last index, which is nulls, adds new item if it is bigger than limit of the memory
            if(tempEnergy+25 < Data.player.energy){
                if (tempNull < 19) {
                    tempEnergy+=25
                    Data.player.chosenSpellsDefense.add(tempNull, Data.player.defaultAttackSpell)
                    Data.player.chosenSpellsDefense.removeAt(Data.player.chosenSpellsDefense.size - 1)
                } else {
                    Data.player.chosenSpellsDefense.add(Data.player.defaultAttackSpell)
                }
            } else break
        }
    }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view = super.onCreateView(inflater, container, savedInstanceState) ?: inflater.inflate(R.layout.fragment_defense, container, false)
        viewTemp = view
        ...
        view.imageViewFragmentAdventureExit.setOnClickListener {
                ...
                init {
                    if(this.index < Data.player.learnedSpells.size){
                        if(Data.player.learnedSpells[this.index] != null){
                            Log.d("fragment_defense_ss", Data.player.learnedSpells[this@Node.index]?.name)
                            Log.d("fragment_defense_spell", Data.player.learnedSpells[this@Node.index]?.bitmapId.toString())
                            component.apply {
                                isEnabled = true
                                isClickable = true
                                setImageBitmap(Data.player.learnedSpells[this@Node.index]?.bitmap)
                            }
                            countText.apply {
                    ...
                    countText.setOnTouchListener(object : Class_OnSwipeTouchListener(context, component, true) {
                        override fun onClick(x: Float, y: Float) {
                            ...
                            imageViewSpellDescription.setImageBitmap(Data.player.learnedSpells[this@Node.index]?.bitmap)
                        }
        ...
        override fun getView(position: Int, convertView: View?, viewGroup: ViewGroup?): View {
            ...
            if(position < Data.player.chosenSpellsDefense.size){
                if(Data.player.chosenSpellsDefense[position]!=null){
                    viewHolder.button1.setImageBitmap(Data.player.chosenSpellsDefense[position]?.bitmap)
            ...
            viewHolder.button1.setOnClickListener {
                ...
                viewHolder.button1.setBackgroundResource(R.drawable.circle_white)
        ...
        when (event.action) {
            DragEvent.ACTION_DRAG_STARTED -> {
                if (event.clipDescription.label == "learned") {
                    ...
                    spell = Data.player.learnedSpells.getOrNull(spellIndex)
