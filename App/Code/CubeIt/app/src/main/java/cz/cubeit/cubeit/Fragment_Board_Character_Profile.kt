    ...
    fun View.getListenerPlayer() {
        ...
        this.setOnTouchListener(object: Class_OnSwipeTouchListener(this.context, this, true){
            ...
            override fun onClick(x: Float, y: Float) {
                super.onClick(x, y)
                if(Data.player.equip[index] != null){
                    (activity as? Activity_Character)?.apply {
                        chooseItem()
                        textViewInfoItemTemp.setHTMLText(Data.player.equip[index]?.getStats() ?: "")
                    }
                }
            }
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        fun View.getListenerBoard(chosenPlayer: Player) {
            ...
            if(chosenPlayer.equip[index] != null){
                this.apply {
                    isClickable = true
                    isEnabled = true
                    val item = chosenPlayer.equip[index] ?: Item()
                    setUpOnHoldDecorPop(tempActivity, true, item.getStatsCompare(), item.getBackground(), item.bitmap)
                }
            }
        }
        ...
        for(i in 0 until 10) {
            ...
            itemEquip.isClickable = if(playerProfile.equip[i]!=null){
                ...
                itemEquip.setBackgroundResource(playerProfile.equip[i]?.getBackground() ?: 0)
                true
            } else {
