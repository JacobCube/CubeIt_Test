...
class ActivityHome : SystemFlow.GameActivity(contentLayoutId = R.layout.activity_home, activityType = ActivityType.Home, hasMenu = false, hasSwipeDown = false) {
                        ...
                        Data.inboxSnapshotHome = docRef.addSnapshotListener(MetadataChanges.INCLUDE) { snapshot, e ->
                            ...
                            if (snapshot != null && !snapshot.isEmpty) {
                                ...
                                for(i in snapshot.documentChanges){
                                    ...
                                    if(i.type == DocumentChange.Type.ADDED)inboxSnap.add(i.document.toObject(InboxMessage::class.java, behaviour))
                                }
                                ...
                                if(snapshot.documents.size >= 1 && inboxSnap.size > 0 && inboxSnap != Data.inbox){
                                    for(i in inboxSnap){
                                        if(!Data.inbox.any { it.id == i.id }  && i.status != MessageStatus.Read){
                                            Data.inbox.add(i)
                                            if(i.status != MessageStatus.Sent && SystemFlow.currentGameActivity?.activityType != ActivityType.Inbox){
                                                Toast.makeText(context, "New message has arrived.", Toast.LENGTH_SHORT).show()
                                                SystemFlow.playComponentSound(context, R.raw.basic_notification)
    ...
    private var rotateAnimationLeave: ObjectAnimator? = null
    ...
    override fun onBackPressed() {
        if(signOut) Data.signOut(this, true)
        else {
            Toast.makeText(genericContext, "Press back again to icon_exit.", Toast.LENGTH_SHORT).show()
            signOut = true
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //Data.player.createFightRequest("EpicGamedManager")

        Data.secondaryInitialization(this)
        ...
        this.window.decorView.rootView.post {
            ...
            var clickableExitHandler = Handler()
                ...
                override fun onTouch(view: View, motionEvent: MotionEvent): Boolean {
                    when (motionEvent.action) {
                        MotionEvent.ACTION_DOWN -> {
                            ...
                            clickableExit = true
                            clickableExitHandler.postDelayed({clickableExit = false}, 100)
                        ...
                        MotionEvent.ACTION_UP -> {
                            ...
                            if(clickableExit){
                                clickableExitHandler.removeCallbacksAndMessages(null)
                                ...
                                if(motionEvent.rawX <= originalXExit - imageViewHomeExit.width * 3){
                        ...
                        MotionEvent.ACTION_MOVE -> {
                            ...
                            if(motionEvent.rawX <= originalXExit - imageViewHomeExit.width * 3){
                                if(rotateAnimationLeave?.isRunning == false){
                                    rotateAnimationLeave = ObjectAnimator.ofFloat(imageViewHomeExit,
                                            "rotation", 0f, 360f)
                                    rotateAnimationLeave?.duration = 250
                                    rotateAnimationLeave?.start()
                                }
                            }else {
                                rotateAnimationLeave?.cancel()
                            }
                            return true
                        }
                    }
