...
// stored drawable IDs are not equal to the drawables anymore, so it changes their final image
        ...
        //others
        , "10000001" to android.R.drawable.ic_menu_report_image
...
open class Player(
    ...
    /**
     * CO-OP offline fight system helper method
     */
    /*fun chooseEnemyOffline(enemies: MutableList<FightSystem.Fighter?>){
        this.chosenEnemyOffline = if(enemies.size == 1){
            enemies.first()
        }else {
            enemies.sortBy { it?.statusWeight ?: 0.0 }
            enemies.first()
        }
    }
    */
    ...
    private fun createInbox(): Task<Task<Void>> {
        ...
        return Data.inbox[0].initialize().continueWith {
            docRef.document().set(Data.inbox[0])
        }
    }
    ...
    fun writeInbox(receiver: String, message: InboxMessage = InboxMessage(sender = this.username, receiver = "MexxFM")): Task<Void> {     //TODO
        ...
        return message.initialize().continueWithTask {
            docRef.document().set(message)
        }
...
class CharClass: Serializable{
    ...
    var inDrawableIcon: String = "00503"
    ...
    @Exclude @Transient var drawableIcon = 0
        @Exclude get() = drawableStorage[inDrawableIcon] ?: 0
...
open class Item(
        ...
        open var drawableIn: String = "",
        open var levelRq: Int = 1,
        open var quality: Int = 1,
        open var charClass: Int = 1,
...
class StorySlide(
    ...
    override fun hashCode(): Int {
        ...
        result = 31 * result + innerInstanceID.hashCode()
    ...
    override fun equals(other: Any?): Boolean {
        ...
        if (innerInstanceID != other.innerInstanceID) return false
...
open class NPC(
        ...
        open var inDrawable: String = "90205",
        open var inBgDrawable: String = "0",
        open var name: String = namesStorage[nextInt(0, namesStorage.size)],
    ...
    @Exclude @Transient var drawable: Int = 0
        @Exclude get() = drawableStorage[inDrawable] ?: charClass.drawable
    var level: Int = 0
    ...
    @Exclude fun generate(difficultyX: Int? = null, playerX: Player, multiplier: Double = 1.0): NPC {
        ...
        tempPlayer.equip = mutableListOf(
                GameFlow.generateItem(playerG = tempPlayer, inQuality = GenericDB.balance.itemQualityGenImpact[this.difficulty.toString()], itemSlot = 0, itemType = ItemType.Weapon)
                , GameFlow.generateItem(playerG = tempPlayer, inQuality = GenericDB.balance.itemQualityGenImpact[this.difficulty.toString()], itemSlot = 1, itemType = ItemType.Weapon)
                , GameFlow.generateItem(playerG = tempPlayer, inQuality = GenericDB.balance.itemQualityGenImpact[this.difficulty.toString()], itemSlot = 2, itemType = ItemType.Wearable)
                , GameFlow.generateItem(playerG = tempPlayer, inQuality = GenericDB.balance.itemQualityGenImpact[this.difficulty.toString()], itemSlot = 3, itemType = ItemType.Wearable)
                , GameFlow.generateItem(playerG = tempPlayer, inQuality = GenericDB.balance.itemQualityGenImpact[this.difficulty.toString()], itemSlot = 4, itemType = ItemType.Wearable)
                , GameFlow.generateItem(playerG = tempPlayer, inQuality = GenericDB.balance.itemQualityGenImpact[this.difficulty.toString()], itemSlot = 5, itemType = ItemType.Wearable)
                , GameFlow.generateItem(playerG = tempPlayer, inQuality = GenericDB.balance.itemQualityGenImpact[this.difficulty.toString()], itemSlot = 6, itemType = ItemType.Wearable)
                , GameFlow.generateItem(playerG = tempPlayer, inQuality = GenericDB.balance.itemQualityGenImpact[this.difficulty.toString()], itemSlot = 7, itemType = ItemType.Wearable)
                , GameFlow.generateItem(playerG = tempPlayer, inQuality = GenericDB.balance.itemQualityGenImpact[this.difficulty.toString()], itemSlot = 8, itemType = ItemType.Wearable)
                , GameFlow.generateItem(playerG = tempPlayer, inQuality = GenericDB.balance.itemQualityGenImpact[this.difficulty.toString()], itemSlot = 9, itemType = ItemType.Wearable)
        )
    ...
    open fun toFighter(type: FightSystem.FighterType): FightSystem.Fighter{
    ...
    fun applyStats(playerX: Player, multiplier: Double = 1.0) {
        val balanceRate: Double = GenericDB.balance.npcrate[this.difficulty.toString()] ?: error("")
    ...
    override fun writeToParcel(parcel: Parcel, flags: Int) {
        ...
        parcel.writeString(inDrawable)
        parcel.writeString(inBgDrawable)
...
class Boss(
        ...
        var surface: Int = 0
): NPC(id, inDrawable, inBgDrawable, name, description, levelAppearance, charClassIndex, difficulty), Parcelable{
    ...
    fun initialize(): Task<Void> {
        ...
        Log.d("boss_multiplier", (nextInt(16 * (this.difficulty!! - 99), 26 * (this.difficulty!! - 99)).toDouble() / 10).toString())
