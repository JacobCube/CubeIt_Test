...
class Activity_Create_Story_Publish: SystemFlow.GameActivity(R.layout.activity_create_story_publish, ActivityType.CreateStoryPublish, hasMenu = false, hasSwipeDown = false){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewPagerCreateStoryPublish.apply {
            setPageTransformer(true, DrawerTransformer())
        }
    }

    class ViewPagerStorySlide internal constructor(fm: FragmentManager, private val storySlides: List<StorySlide>) : FragmentPagerAdapter(fm){

        override fun getItem(position: Int): Fragment {
            return Fragment_Story_Slide.newInstance(storySlides[position])
        }
